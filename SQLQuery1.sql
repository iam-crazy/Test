...
BEGIN
  ...
  SET @ROW_NUMBER = 0;
	   SELECT ( @ROW_NUMBER := @ROW_NUMBER + 1) AS SNo ,A.* FROM job_purposedetails A WHERE A.ChallanNo=$ChallanNo and a.active=1;
...
begin
  ...
  elseif ($KeyCode ='Result') then
   SET @ROW_NUMBER = 0;
        ...
        left join watermanagementsystem.parameters b
        on a.Id=b.Id where a.SampleNumber=$SampleNumber order by b.Id Asc;
  elseif ($KeyCode ='ChallanReport') then
    select SampleNo as  SampleNumber,
PurposeParameter as Result,
Purpose as Parameter,

concat(Amount) as PermissableLimit from  job_purposedetails where ChallanNo =$SampleNumber;
  else
   begin    
        SET @ROW_NUMBER = 0;
          SELECT
...
DROP PROCEDURE IF EXISTS watermanagementsystem.Wms_ChallanStatusUpdate;
CREATE PROCEDURE watermanagementsystem.`Wms_ChallanStatusUpdate`(
$ChallanNo varchar(100),
OUT $OUTPUT  VARCHAR(200)
)
BEGIN
    update watermanagementsystem.job_challandetails set Status='Consumer Received' where  ChallanNo=$ChallanNo;
    update watermanagementsystem.job_purposedetails set Status='Consumer Received' where  ChallanNo=$ChallanNo;
...
$CHALLAN_FROM_DATE VARCHAR(100), 
$CHALLAN_TO_DATE VARCHAR(100), 
...
BEGIN 
  SET @FILTER:=''; 
    ...
    ELSEIF $KEYCODE ='CHALLANNO' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO,'  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE' THEN 
     SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")','   ORDER BY A.ID DESC')
      INTO   @FILTER;  
    ...
    ELSEIF $KEYCODE ='STATUS' THEN 
      SELECT CONCAT(' WHERE  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC') 
      INTO   @FILTER; 
    ...
    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")','  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_STATUS' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_STATUS' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO_STATUS' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO_STATUS_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO_STATUS_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_CHALLANDATE_SAMPLENO_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 
    ...
    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_STATUS' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_STATUS_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_STATUS_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_STATUS_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLENO_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_STATUS_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_STATUS_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_STATUS_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANNO_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  A.ChallanKey   >=', $CHALLAN_FROM_NO,' AND A.ChallanKey<=', $CHALLAN_TO_NO, ' AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_STATUS' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_STATUS' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_STATUS_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_STATUS_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_STATUS_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='CHALLANDATE_SAMPLENO_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE DATE_FORMAT(A.ChallanDate, "%Y-%m-%d")   between DATE_FORMAT(STR_TO_DATE("', $CHALLAN_FROM_DATE,'", "%d-%m-%Y"), "%Y-%m-%d") and   DATE_FORMAT(STR_TO_DATE("',$CHALLAN_TO_DATE,'", "%d-%m-%Y"), "%Y-%m-%d")',' AND  B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='SAMPLENO_STATUS' THEN 
      SELECT CONCAT(' WHERE B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 
    ...
    ELSEIF $KEYCODE ='SAMPLENO_STATUS_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,'  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND  A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='SAMPLENO_STATUS_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE (B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,')  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='SAMPLENO_STATUS_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE (B.SampleId   >=', $SAMPLE_FROM_NO,' AND B.SampleId <=', $SAMPLE_TO_NO,')  AND  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND  A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 
    ...
    ELSEIF $KEYCODE ='STATUS_SAMPLETYPE' THEN 
      SELECT CONCAT(' WHERE  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'"  ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='STATUS_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 

    ELSEIF $KEYCODE ='STATUS_SAMPLETYPE_OBSERVATION' THEN 
      SELECT CONCAT(' WHERE  IFNULL(B.STATUS,"Challan Created")="', $STATUS,'" AND A.SAMPLETYPE="', $SAMPLE_TYPE,'" AND B.OBSERVATION="', $OBSERVATION,'"   ORDER BY A.ID DESC')
      INTO   @FILTER; 
    ...
    SET @ROW_NUMBER = 0;
    SET @QUERY='SELECT A.ID,CAST(A.CHALLANNO AS CHAR(1000)) AS CHALLANNO,A.CHALLANDATE,A.SAMPLETYPE,B.PURPOSE,B.SAMPLEno as SampleName,A.SAMPLECOLLECTEDON,A.SAMPLESENTBY,A.ADDRESS,A.CREATEDBY,A.CREATEDON,IFNULL(B.STATUS,"Challan Created") as STATUS,IFNULL(A.STATUS,"Challan Created") as ChallanStatus,B.OBSERVATION FROM JOB_CHALLANDETAILS A LEFT JOIN JOB_PURPOSEDETAILS B ON A.ID = B.CHALLANDETAILSID AND A.CHALLANNO = B.CHALLANNO';
...
begin
  IF $ISSAVE =1 THEN
    ...
    END;
      ...
      update JOB_PURPOSEDETAILS set 
      PURPOSE= $PURPOSE,
      PROPOSEPARAMETER=$PROPOSEPARAMETER,
      Location= $Location,
...
begin
  ...
  elseif  $KeyCode='ChallanReport' then
   begin 
     SELECT SampleNo as SampleNumber, Purpose as Parameter, PurposeParameter as Result,concat( Amount ) as PermissableLimit, TypeOfBuilding AS AcceptableLimit, Location  AS ChallanNo
FROM watermanagementsystem.job_purposedetails where ChallanNo=$SampleNumber;
  end;
  else 
      begin
